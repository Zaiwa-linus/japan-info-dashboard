<script>
    /**
     * @typedef {Object} TileRow
     * @property {string} area_code - 都道府県コード (e.g. '01000')
     * @property {string} area_name - 都道府県名
     * @property {number|null} value - 表示する値
     */

    /** @type {TileRow[]} data - area_code, area_name, value を含むクエリ結果 */
    export let data = [];
    ...
    function ratio(val) {
        ...
        return (Number(val) - minVal) / range;
    }

    function borderColor(val) {
        if (val == null) return '#e2e8f0';
        const r = ratio(val);
        return `rgb(${Math.round(255 - r * 196)}, ${Math.round(255 - r * 125)}, ${Math.round(255 - r * 9)})`;
    }
...
<div class="tile-map">
    {#each tiles as tile}
        <div
            ...
            style="grid-row: {tile.row}; grid-column: {tile.col}; border-color: {borderColor(tile.value)}; border-width: {tile.isSelected ? '2.5px' : '1.5px'};"
        >
