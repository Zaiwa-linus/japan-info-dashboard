...
python Estat/search_stats.py "観光" --field 11

# 調査年で絞り込み
python Estat/search_stats.py "労働力" --years 2023

# 取得件数を変更（デフォルト50件）
python Estat/search_stats.py "GDP" --limit 20

# 統計分野コード一覧を表示
python Estat/search_stats.py --list-fields

# JSON形式で出力
python Estat/search_stats.py "人口" --json
```
...
```bash
# デフォルトパス（data/{統計表ID}/{統計表ID}.csv）に保存
python Estat/download_data.py 0003317252

# 出力先を指定
python Estat/download_data.py 0003317252 -o data/custom_dir/population.csv
```
