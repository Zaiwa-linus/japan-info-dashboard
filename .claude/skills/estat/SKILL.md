...
データを探す、ダウンロードするように指示を受けたら基本的にはこのライブラリを使うこと。
...
```bash
# キーワードで検索
uv run python .claude/skills/estat/search_stats.py "人口"

# 統計分野コードで絞り込み（例: 11=運輸・観光）
uv run python .claude/skills/estat/search_stats.py "観光" --field 11

# 調査年で絞り込み
uv run python .claude/skills/estat/search_stats.py "労働力" --years 2023

# 取得件数を変更（デフォルト50件）
uv run python .claude/skills/estat/search_stats.py "GDP" --limit 20

# 統計分野コード一覧を表示
uv run python .claude/skills/estat/search_stats.py --list-fields

# JSON形式で出力
uv run python .claude/skills/estat/search_stats.py "人口" --json
```
...
```bash
# メタ情報を表示
uv run python .claude/skills/estat/get_meta.py 0003317252

# JSON形式で出力
uv run python .claude/skills/estat/get_meta.py 0003317252 --json
```
...
- 詳細仕様は `.claude/skills/estat/e-stat-api-3.0.md` を参照
